...
need_grep '^P2P_PORT=8155$' ".env.example"
need_grep 'P2P_PORT: \$\{P2P_PORT:-8155\}' "docker-compose.yml"
need_grep '^port=8150$' "node.conf.example"
reject_grep '^port=8154$' "node.conf.example"
need_grep '^BDAG_NODE_OBSOLETE_HEIGHT=20$' ".env.example"
need_grep '^BDAG_NODE_MINING_NO_PENDING_TX=0$' ".env.example"
need_grep 'BDAG_NODE_OBSOLETE_HEIGHT: \$\{BDAG_NODE_OBSOLETE_HEIGHT:-20\}' "docker-compose.yml"
need_grep 'BDAG_NODE_MINING_NO_PENDING_TX: \$\{BDAG_NODE_MINING_NO_PENDING_TX:-0\}' "docker-compose.yml"
need_grep '^BDAG_NODE_RPC_URLS=node=http://127\.0\.0\.1:38131$' ".env.example"
reject_grep '^BDAG_NODE_RPC_URLS=node=http://node:38131$' ".env.example"
	...
	need_grep '^NODE_DATA_DIR=./data/node$' ".env.example"
	need_grep '^MINING_POOL_ADDRESS=$' ".env.example"
	reject_grep '^MINING_POOL_ADDRESS=0x[0-9a-fA-F]{40}$' ".env.example"
	need_grep '\$\{NODE_DATA_DIR:-\./data/node\}:/var/lib/bdagStack/node' "docker-compose.yml"
	need_grep 'dashboard_src: \$\{DASHBOARD_SRC_CONTEXT:-\.\./redis-dash\}' "docker-compose.yml"
	need_grep '^POOL_ASIC_ARP_TABLE_PATH=/host/proc/net/arp$' ".env.example"
	need_grep '\$\{BDAG_HOST_ARP_TABLE_PATH:-/proc/net/arp\}:/host/proc/net/arp:ro' "docker-compose.yml"
	need_grep 'POOL_ASIC_ARP_TABLE_PATH: .*POOL_ASIC_ARP_TABLE_PATH' "docker-compose.yml"
	need_grep 'POOL_ASIC_MAC_ALLOWLIST: .*POOL_ASIC_MAC_ALLOWLIST' "docker-compose.yml"
...
need_grep 'BOOTSTRAP_PEER_ADDRESSES: \$\{BOOTSTRAP_PEER_ADDRESSES:-\}' "docker-compose.yml"
	...
	need_grep '^BDAG_MINING_IMPERATIVE_REPAIR_ENABLED=1$' ".env.example"
	need_grep '^BDAG_MINING_IMPERATIVE_GUARD_CONTAINERS=watchdog,sentinel$' ".env.example"
	need_grep 'BDAG_MINING_IMPERATIVE_GUARD_CONTAINERS: \$\{BDAG_MINING_IMPERATIVE_GUARD_CONTAINERS:-watchdog,sentinel\}' "docker-compose.yml"
	need_grep 'compose_cmd up -d --no-build --pull never pool-db node dashboard status-sampler watchdog sentinel' "ops/release-install.sh"
	need_grep 'docker compose up -d --no-build --pull never pool-db node dashboard status-sampler watchdog sentinel' "scripts/release/installers/install-unix-common.sh"
	need_grep 'docker compose up -d --no-build --pull never pool-db node dashboard status-sampler watchdog sentinel' "scripts/release/installers/install-windows.ps1"
	need_grep '^POOL_STRATUM_SERVER_FIRST_DIFFICULTY_PROBE=false$' ".env.example"
	need_grep '^POOL_MIN_PDIFF=0\.05$' ".env.example"
	need_grep '^POOL_STARTING_PDIFF=0\.05$' ".env.example"
	need_grep 'POOL_STRATUM_SERVER_FIRST_DIFFICULTY_PROBE: \$\{POOL_STRATUM_SERVER_FIRST_DIFFICULTY_PROBE:-false\}' "docker-compose.yml"
	need_grep 'POOL_MIN_PDIFF: \$\{POOL_MIN_PDIFF:-0\.05\}' "docker-compose.yml"
	need_grep 'POOL_STARTING_PDIFF: \$\{POOL_STARTING_PDIFF:-0\.05\}' "docker-compose.yml"
	need_grep '^POOL_RECENT_STALE_BLOCK_CANDIDATE_SUBMIT_GRACE_MS=1750$' ".env.example"
...
need_grep 'First-Install Mining Safety Gates' "README.md"
need_grep 'native `getTemplateHealth`' "README.md"
need_grep 'Node config edits and service recreates are cold/idle operations' "README.md"
need_grep 'node=http://127\.0\.0\.1:38131' "README.md"
need_grep '2026-06-20 Hardening Addendum' "docs/redis-dash-deploy-difficulties-2026-06-19.md"
need_grep 'P2P Port Drift Was the Root Failure' "docs/redis-dash-deploy-difficulties-2026-06-19.md"
need_grep 'Dashboard Sync Status Must Follow Native Template Health' "docs/redis-dash-deploy-difficulties-2026-06-19.md"
need_grep '2026-06-21 Live Node Mutation Incident' "docs/redis-dash-deploy-difficulties-2026-06-19.md"
need_grep 'Do Not Source Complex `[.]env` Files In Shell Checks' "docs/redis-dash-deploy-difficulties-2026-06-19.md"
need_grep 'Live node mutation guard' "docs/release-readiness-gates.html"
need_grep 'template coinbase .* expected' "scripts/release-readiness-check.py"
need_grep 'missing non-zero expected mining address' "scripts/release-readiness-check.py"
need_grep 'getBlockTemplate.*expected payout address' "docs/release-readiness-gates.html"
