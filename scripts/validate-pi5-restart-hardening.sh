...
need_grep 'BDAG_NODE_SERVICES.*node' "ops/release-install.sh"
need_grep 'BDAG_STACK_SERVICES.*postgres,node,pool' "ops/release-install.sh"
need_grep 'POOL_RPC_BACKENDS.*node=http://127.0.0.1:38131' "ops/release-install.sh"
...
need_grep 'DEFAULT_ACTIVE_NODE_SERVICES = \["node"\]' "ops/update-local-peers.py"
...
need_grep 'BDAG_FASTSYNC_RANGE_BLOCKS=' ".env.example"
need_grep 'BDAG_FASTSYNC_PREPROCESS_WORKERS=' ".env.example"
...
need_grep 'SYNC_SOURCE_NODE=' ".env.example"
...
need_grep 'BDAG_GLOBAL_HISTORY_COMPACT_MULTIPLIER=' ".env.example"
	...
	need_grep 'POOL_RPC_ROUTER_NODE_HEALTH_FRESH_TEMPLATE_GRACE_SECONDS=' ".env.example"
	need_grep 'POOL_RPC_ROUTER_NODE_HEALTH_MIN_CONSENSUS_PEERS=' ".env.example"
	need_grep 'POOL_SUBMIT_STALE_BLOCK_CANDIDATES=' ".env.example"
	need_grep 'POOL_RECENT_STALE_BLOCK_CANDIDATE_SUBMIT_GRACE_MS=' ".env.example"
	...
	need_grep 'POOL_AUTO_TUNE_BLOCK_CANDIDATE_MAX_AGE_MS=' ".env.example"
...
need_grep 'POOL_EXPIRED_JOB_CLIENT_RECONNECT_WINDOW_SECONDS=' ".env.example"
	...
	need_grep 'POOL_PREEMPTIVE_BLOCK_CANDIDATE_CLEAN_REISSUE_ENABLED=' ".env.example"
	need_grep 'POOL_PREEMPTIVE_BLOCK_CANDIDATE_REFRESH_DELAY_MS=' ".env.example"
	need_grep 'POOL_PREEMPTIVE_BLOCK_CANDIDATE_REFRESH_INTERVAL_MS=' ".env.example"
	need_grep 'POOL_PREEMPTIVE_BLOCK_CANDIDATE_REFRESH_TIMEOUT_MS=' ".env.example"
need_grep 'NODE_RPC_URLS=http://127.0.0.1:38131' ".env.example"
need_grep 'POOL_SUBMIT_RPC_URLS=' ".env.example"
need_grep 'NODE_RPC_URLS: .*http://127.0.0.1:38131' "docker-compose.yml"
	...
	need_grep 'POOL_RPC_ROUTER_NODE_HEALTH_MIN_CONSENSUS_PEERS: .*POOL_RPC_ROUTER_NODE_HEALTH_MIN_CONSENSUS_PEERS' "docker-compose.yml"
	need_grep 'POOL_SUBMIT_STALE_BLOCK_CANDIDATES: .*POOL_SUBMIT_STALE_BLOCK_CANDIDATES' "docker-compose.yml"
	need_grep 'POOL_RECENT_STALE_BLOCK_CANDIDATE_SUBMIT_GRACE_MS: .*POOL_RECENT_STALE_BLOCK_CANDIDATE_SUBMIT_GRACE_MS' "docker-compose.yml"
...
need_grep 'BDAG_NODE_SERVICES=node' ".env.example"
...
need_grep 'DEFAULT_NODE_CHILD_GUARD_NODES = "node"' "host/mining-appliance/bdag-node-child-guard"
...
need_grep 'BDAG_NODE_SERVICES=node' "ops/systemd/bdag-status-sampler.service"
...
need_grep 'compose_cmd up -d --no-build --pull never pool-db node dashboard' "ops/release-install.sh"
...
if [[ "$mode" == "source" ]]; then
  need_grep 'stage_snapshot_for_node_datadir' "scripts/release/installers/install-unix-common.sh"
  need_grep 'Stage-SnapshotForNodeDatadir' "scripts/release/installers/install-windows.ps1"
  ...
  need_grep 'docker compose up -d --no-build --pull never pool-db node dashboard' "scripts/release/installers/install-unix-common.sh"
  ...
  need_grep 'c74f88b9c1b4fbf4213e15272d3bf1f63943e839' ".github/workflows/build.yml"
...
need_grep 'ensure_stack_default_env_value BDAG_NODE_SERVICES' "ops/install-dashboard.sh"
