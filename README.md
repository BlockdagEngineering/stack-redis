...
`dockerfile`, `.env.example`, `docker/`, and the cross-platform payload
installers. The release workflow builds `dashboard` from
`BlockdagEngineering/redis-dash`; collector source is not checked out,
packaged, or run by this product.
...
payload exposes RPC latency and Linux IO pressure metrics. When PSI is unavailable, the status sampler falls back to `/proc/stat`
...
The dashboard UI and status API are exposed on `DASHBOARD_HOST_PORT` (`8088` by
default). Global production data must be sourced from native BlockDAG chain RPC
`getBlockCount`/ordered block/coinbase calls. EVM RPC belongs to wallet balance
views only. The packaged Redis dashboard is the stack status surface for this
product.

When testing directly from a source checkout, keep `DASHBOARD_SRC_CONTEXT`
pointing at a local `BlockdagEngineering/redis-dash` clone, normally
`../redis-dash` beside this repo.
...
The dashboard and ops runtime use Python or Go HTTP clients for local status
and pool metrics. Do not make live status depend on host utilities such as
`curl`; release packages should behave the same on Linux AMD64, Linux ARM64,
macOS Docker Desktop, and Windows Docker Desktop once Docker and Python are
available.
